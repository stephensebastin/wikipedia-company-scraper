...
SCAPE_DELAY = 1000 # Wikipedia has a one page per second scrape limit
    ...
    if child_node.name == 'h2' # set current state when we pass a state header
      ...
      if child_node.matches?('div.hatnote') # if state has specific pages
        child_node.css('a').each do |a|
          state_links << a.attributes['href'].value # these pages are all slightly different and will require more parsing strategies
        end
